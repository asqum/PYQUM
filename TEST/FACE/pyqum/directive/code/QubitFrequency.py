...
class Db_Scan():
  def __init__(self,quantificationObj):#,Ec,status,area_Maxratio,density
  ...
  def do_analysis(self):
    self.freq = self.quantificationObj.rawData["x"]  #for qubit  <b>XY-Frequency(GHz)</b>
    I = self.quantificationObj.rawData["iqSignal"][0].real
    Q = self.quantificationObj.rawData["iqSignal"][0].imag

    inp_db = []
    for i in range(I.shape[0]):
      inp_db.append(list(np.hstack(([I[i]],[Q[i]]))))

    # start DBSCAN
    eps,min_samples = Find_eps(inp_db)
    labels_db = dbscan(np.array(inp_db),eps,min_samples)
  ...
  def plot_result(self):
    ...
    plt.plot(self.freq,self.sub,'b-',label=self.title)
    plt.plot(self.freq[farest],self.sub[farest],'rx',label='farest')

    plt.title(self.title)
    plt.xlabel('XY-Frequency (GHz)')
    plt.legend()
    plt.show()