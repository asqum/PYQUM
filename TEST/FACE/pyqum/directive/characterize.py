...
def F_Response(user, tag="", corder={}, comment='', dayindex='', taskentry=0, resumepoint=0, instr=['YOKO','ENA'], testeach=False):
    ...
    while True:
        for i in measure_loop_1:
            ...
            caddress = cdatasearch(i, [fluxbias.count,Sparam.count,ifb.count,powa.count])

            # setting each c-order (From High to Low level of execution):
            if not i%prod([Sparam.count,ifb.count,powa.count]): # virtual for-loop using exact-multiples condition
            ...
            if not i%prod([ifb.count,powa.count]): # virtual for-loop using exact-multiples condition
                ENA.setrace(bench, Mparam=[Sparam.data[caddress[1]]], window='D1')

            if not i%prod([powa.count]): # virtual for-loop using exact-multiples condition
...
def CW_Sweep(user, tag="", corder={}, comment='', dayindex='', taskentry=0, resumepoint=0, instr=['YOKO','ENA'], testeach=False):
    ...
    ifb = waveform(corder['IF-Bandwidth'])
    freq = waveform(corder['Frequency'])
    # special treatment to power parameter: (detecting inner-repeat)
    if 'r' in corder['Power'].lower():
        try: 
            powa_step, powa_repeat = corder['Power'].lower().split('r')
            corder['Power'] = powa_step # corrected back after retrieved repeat-count
            while " " in powa_repeat:
                powa_repeat.replace(" ","")
            powa_repeat = int(powa_repeat)
        except(ValueError): powa_repeat = 1
    powa = waveform(corder['Power'])
    # Total data points:
    datasize = prod([waveform(x).count for x in corder.values()]) * powa_repeat * 2 #data density of 2 due to IQ
    ...
    if powa_repeat == 1: 
        ...
        buffersize_1 = 2 # (buffer) data density of 2 due to IQ
    ...
    while True:
        for i in measure_loop_1:
            ...
            if not i%prod(cstructure[4::]): # virtual for-loop using exact-multiples condition
                ENA.cwfreq(bench, action=['Set', freq.data[caddress[3]]])
