...
# y: freq, x: fluxbias
def fresp_assembler_freq_fluxbias(args):
	(y,x) = args
...
# y: freq, x: powa
def fresp_assembler_freq_powa(args):
	(y,x) = args
...
def fresp_scanner(a, b):
	for i in a:
		for j in b:
			yield i, j
def worker_fresp(y_count,x_count,y="freq",x="fluxbias"):		
	pool = Pool()
	IQ = pool.map(eval("fresp_assembler_%s_%s" %(y,x)), fresp_scanner(range(y_count),range(x_count)), max(x_count,y_count))
