...
def char_cwsweep_access():
	...
	session['c_cwsweep_structure'] = [cfluxbias.count,csparam.count,cifb.count,cpowa.count,cfreq.count*M_cwsweep[session['user_name']].datadensity]
	...
	cpowa_data = cpowa.data[0:session['c_cwsweep_address'][3]+1]
	cfreq_data = cfreq.data # within buffer
...
def char_cwsweep_1ddata():
	...
	if ifluxbias == "x":
		...
		selected_I = [selectedata[gotocdata([x, int(isparam), int(iifb), int(ipowa), 2*int(ifreq)], session['c_cwsweep_structure'])] for x in range(session['c_cwsweep_address'][0]+1)]
		selected_Q = [selectedata[gotocdata([x, int(isparam), int(iifb), int(ipowa), 2*int(ifreq)+1], session['c_cwsweep_structure'])] for x in range(session['c_cwsweep_address'][0]+1)]
	elif isparam == "x":
		pass
	elif iifb == "x":
		pass
		...
		selected_I = [selectedata[gotocdata([int(ifluxbias), int(isparam), int(iifb), x, 2*int(ifreq)], session['c_cwsweep_structure'])] for x in range(session['c_cwsweep_address'][3]+1)]
		selected_Q = [selectedata[gotocdata([int(ifluxbias), int(isparam), int(iifb), x, 2*int(ifreq)+1], session['c_cwsweep_structure'])] for x in range(session['c_cwsweep_address'][3]+1)]
	elif ifreq == "x":
		title = "<b>frequency(GHz)</b>"
		selected_sweep = M_cwsweep[session['user_name']].corder['Frequency']
		selected_progress = waveform(selected_sweep).data # Full sweep
		selected_I = [selectedata[gotocdata([int(ifluxbias), int(isparam), int(iifb), int(ipowa), 2*x], session['c_cwsweep_structure'])] for x in range(waveform(selected_sweep).count)]
		selected_Q = [selectedata[gotocdata([int(ifluxbias), int(isparam), int(iifb), int(ipowa), 2*x+1], session['c_cwsweep_structure'])] for x in range(waveform(selected_sweep).count)]
	MagPha = [IQAP(x[0],x[1]) for x in zip(selected_I, selected_Q)]
	Amp, Pha = [], []
	for i,j in MagPha:
		Amp.append(i); Pha.append(j)
...
def char_cwsweep_2ddata():
	...
	session['ifreq'] = request.args.get('ifreq')
	x, y, ZZ = [], [], []
	if session['ifluxbias'] == "x" and session['ifreq'] == "y":
		...
		# rI, rQ, Amp, pha = worker_cwsweep(x_count, y_count)
		# CMD = ["python", "-c", "from pyqum.directive import multiprocessor as mp; mp.worker_cwsweep(%s,%s,%s)"%(10000,1000,list(range(10000000*2)))]
		CMD = ["python", "multiprocessor.py", 10000, 1000, list(range(10000000*2))]
