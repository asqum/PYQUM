...
$(document).ready(function(){
    ...
    PD_render_input("powerdepend");
    console.log( "Load powerdepend" );

    let e_loadButton = document.getElementById("powerdepend-button-load");
    e_loadButton.addEventListener("click", PD_load_data);
    let e_fitButton = document.getElementById("powerdepend-button-fit");
    e_fitButton.addEventListener("click", PD_fit_data);
    let e_plot1DButton = document.getElementById("powerdepend-plot1D-button");
    e_plot1DButton.addEventListener("click", PD_get_plot1D);
    let e_plot2DtButton = document.getElementById("powerdepend-plot2D-button");
    e_plot2DtButton.addEventListener("click", PD_get_plot2D);
});

function get_powerdepend_selectInfo( quantificationType ){
...
function PD_render_input ( quantificationType )
{
    ...
    // PD_creatDOM_FitDataType("all", "powerdepend-allParamater");
    $.ajaxSettings.async = true;
}
...
function PD_creatDOM_FitDataType( fitFunc, parentID ){
    // Creat select for parameter plot type
    console.log( "PD_creatDOM_FitDataType" );
...
function PD_load_data(){
    ...
    $.ajaxSettings.async = false;
    let analysisIndex = get_powerdepend_selectInfo("powerdepend");

    //Send information to python
    $.getJSON( '/benchmark/powerdepend/load',
        {   quantificationType: JSON.stringify("powerdepend"), 
...
function PD_get_plot2D(){
    console.log( "Plot data" );

    let plotID_2D = "powerdepend-plot2D-rawOverview";
    let plotID_1D_ampPhase = "powerdepend-plot1D-ampPhase";
    let plotID_1D_IQ = "powerdepend-plot1D-IQ";
    $.ajaxSettings.async = false;
    let htmlInfo=get_htmlInfo_python();


    let plot2D_signalType = document.getElementById("powerdepend-plot2D-zSelector").value;
    let plot1D_yAxisType = document.getElementById("powerdepend-plot2D-ySelector").value;
    ...
    //Get 2D data
    $.getJSON( '/benchmark/powerdepend/getJson_plot2D',
    ...
    //Get x axis
    $.getJSON( '/benchmark/powerdepend/getJson_plotAxis',
...
function PD_get_plot1D(){
    ...
    let selectType = document.getElementById("powerdepend-plot2D-ySelector").value;
    let selectValue = document.getElementById("powerdepend-plot1D-y_value").value;
    ...
    //Get raw signal
    $.getJSON( '/benchmark/powerdepend/getJson_plot1D',
...
function PD_fit_data(){

    // Plot fit parameters
    $.getJSON( '/benchmark/powerdepend/getJson_fitParaPlot',{ }, function (data) {});
}
