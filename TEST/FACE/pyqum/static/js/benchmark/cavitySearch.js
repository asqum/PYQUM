...
$(document).ready(function(){
    ...
    render_selection("cavitySearch");
    console.log( "Load cavitySearch" );
});
...
$(function () {
    ...
    // plot
    $('#cavitySearch-plot-button').on('click', function () {
        let plotID_2D = "cavitySearch-plot2D-rawOverview";
        let plotID_1D_ampPhase = "cavitySearch-plot1D-ampPhase";
        let plotID_1D_IQ = "cavitySearch-plot1D-IQ";
        console.log( "plot!!" );
        $.ajaxSettings.async = false;
        let htmlInfo = get_htmlInfo_python();
        let analysisIndex = get_selectInfo("cavitySearch");
        if ( analysisIndex.axisIndex.length == 2 ){
            $.getJSON( '/benchmark/cavitySearch/getJson_plot',
            {   quantificationType: JSON.stringify("cavitySearch"), analysisIndex: JSON.stringify(analysisIndex), plotType: JSON.stringify("2D_amp"), },
                function (data) {
                console.log( "2D plot" );
                console.log( data );
                let axisKeys = {
    ...
    //Test fit data
    $('#cavitySearch-fit-button').on('click', function () {
        ...
        let fitRange = document.getElementById("cavitySearch"+"-fitting_input-"+xAxisKey).value;
        ...
        $.getJSON( '/benchmark/qestimate/getJson_fitParaPlot',{  
            ...
            plot1D( plotdata, axisKeys_fitResult, "cavitySearch-plot-fittingParameters");
