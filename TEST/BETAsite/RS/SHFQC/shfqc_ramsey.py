...
RAMSEY_PULSE_DURATION = 50e-9
START_ZERO_DURATION = 96e-9 # multiples of 8 ns
STEP_ZERO_DURATION = 32e-9 # 8 ns resolution
N_STEP = 10
SHFSG_SAMPLING_FREQUENCY = 2e9

start_zero_len = int(START_ZERO_DURATION * SHFSG_SAMPLING_FREQUENCY)
step_zero_len = int(STEP_ZERO_DURATION * SHFSG_SAMPLING_FREQUENCY)

SG_SEQUENCER_CODE = f"""
// Define constants in time domain
const t_len = {RAMSEY_PULSE_DURATION};
const amp = 1;
const n_steps = {N_STEP};
const n_aves = 1;
...
const s_start_zero_len = round({start_zero_len}/16)*16;
const s_step_zero_len = round({step_zero_len}/16)*16;
const s_readout = round(s_rate*t_readout/16)*16;

// Define waveform and assign index
wave w = gauss(s_len, amp, s_len/2, s_len/8);
...
{{ 
  ...
  executeTableEntry(0);
  playZero(s_start_zero_len+i*s_step_zero_len);
  executeTableEntry(0);
  //Readout window
  playZero(s_readout);
}}
...
{{
  ...
  {{  
        ...
        playZero(s_len+s_start_zero_len+i*s_step_zero_len+s_len);
        playZero(s_readout);
