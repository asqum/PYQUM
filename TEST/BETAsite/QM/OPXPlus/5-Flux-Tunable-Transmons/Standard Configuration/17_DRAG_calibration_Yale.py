...
with program() as drag:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(a, amps)):
            ...
            play("x180" * amp(1, 0, 0, a), f"q{qubit}_xy")
            play("y90" * amp(a, 0, 0, 1), f"q{qubit}_xy")
            ...
            state, I, Q = readout_macro(f"rr{qubit}", multiplexed, threshold=ge_threshold_q5, state=state, I=I, Q=Q)
            ...
            play("y180" * amp(a, 0, 0, 1), f"q{qubit}_xy")
            play("x90" * amp(1, 0, 0, a), f"q{qubit}_xy")
            # Align the two elements to measure after playing the qubit pulses.
            align(f"q{qubit}_xy", f"rr{qubit}")
            # Measure the resonator and extract the qubit state
            state, I, Q = readout_macro(f"rr{qubit}", multiplexed, threshold=ge_threshold_q5, state=state, I=I, Q=Q)
