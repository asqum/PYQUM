...
# The QUA program #
###################
qubit_in_e = 1  # Qubit number to put in |e> at the beginning of the sequence
qubit_to_flux_tune = 2  # Qubit number to flux-tune

# Flux offset
flux_bias = config["controllers"]["con2"]["analog_outputs"][
    config["elements"][f"q{qubit_to_flux_tune}_z"]["singleInput"]["port"][1]
]["offset"]

n_avg = 130000  # The number of averages
ts = np.arange(4, 120, 1)  # The flux pulse durations in clock cycles (4ns) - Must be larger than 4 clock cycles.
# amps = (np.arange(0, 0.5, 0.001) - flux_bias) / const_flux_amp  # The flux amplitude pre-factor
amps = (np.arange(0.25, 0.28, 0.0001) - flux_bias) / const_flux_amp  # The flux amplitude pre-factor
