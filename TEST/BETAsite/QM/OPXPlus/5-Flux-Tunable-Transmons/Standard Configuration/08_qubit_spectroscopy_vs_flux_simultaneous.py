    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(df, dfs)):
            ...
            with for_(*from_array(dc, dcs)):
                ...
                multiplexed_readout(I, I_st, Q, Q_st, resonators=[1, 2], amplitude=0.99)
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.subplot(231)
        plt.cla()
        plt.pcolor(dcs, (dfs + qubit_IF_q1) / u.MHz, R1)
        plt.xlabel("flux [V]")
        plt.ylabel("q1 dIF [MHz]")
        plt.title(f"q1 (f_res: {(qubit_LO_q1 + qubit_IF_q1) / u.MHz} MHz)")
        plt.subplot(234)
        plt.cla()
        plt.pcolor(dcs, (qubit_LO_q1 + qubit_IF_q1 + dfs) / u.MHz, phase1)
        plt.xlabel("flux [V]")
        plt.ylabel("q1 freq [MHz]")
        plt.subplot(232)
        plt.cla()
        plt.pcolor(dcs, (dfs + qubit_IF_q2) / u.MHz, R2)
        plt.title(f"q2 (f_res: {(qubit_LO_q2 + qubit_IF_q2) / u.MHz} MHz)")
        plt.xlabel("Flux [V]")
        plt.ylabel("q2 dIF [MHz]")
        plt.subplot(235)
        plt.cla()
        plt.pcolor(dcs, (qubit_LO_q2 + qubit_IF_q2 + dfs) / u.MHz, phase2)
        plt.xlabel("Flux [V]")
        plt.ylabel("q2 freq [MHz]")
        # Add both to compare:
        plt.subplot(233)
        plt.cla()
        plt.pcolor(dcs, (qubit_LO_q2 + qubit_IF_q2 + dfs) / u.MHz, np.abs(S1) + np.abs(S2))
        plt.title(f"Sum of the two resonators")
        plt.xlabel("Flux [V]")
        plt.ylabel("q1=q2 freq [MHz]")
        plt.subplot(236)
        plt.cla()
        plt.pcolor(dcs, (qubit_LO_q2 + qubit_IF_q2 + dfs) / u.MHz, np.unwrap(np.angle(S1)) + np.unwrap(np.angle(S2)))
        plt.xlabel("Flux [V]")
        plt.ylabel("q1=q2 freq [MHz]")
        plt.tight_layout()
        plt.pause(1)
