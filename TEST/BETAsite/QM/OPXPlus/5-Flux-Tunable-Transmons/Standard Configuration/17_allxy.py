...
def allXY(pulses, qubit, resonator):
    ...
    **Example:** I, Q = allXY(['I', 'y90'])
    ...
    """
    I_others = declare(fixed)
    Q_others = declare(fixed)
    I_xy = declare(fixed)
    Q_xy = declare(fixed)
    if pulses[0] != "I":
...
if simulate:
    ...
    while results.is_processing():
        ...
        plt.subplot(2,1,1)
        plt.cla()
        plt.plot(I, "-bx", label="data")
        plt.plot([np.max(I)] * 5 + [(np.mean(I))] * 12 + [np.min(I)] * 4, "r-", label="Goal")
        plt.ylabel("I [a.u.]")
        plt.xticks(ticks=range(len(sequence)), labels=["" for _ in sequence], rotation=45)
        plt.legend()
        plt.subplot(2,1,2)
        plt.cla()
        plt.plot(Q, "-bx", label="data")
        plt.plot([np.max(Q)] * 5 + [(np.mean(Q))] * 12 + [np.min(Q)] * 4, "r-", label="Goal")
        plt.ylabel("Q [a.u.]")
    ...
    fig = plt.figure()
    plt.cla()
    plt.plot(I, "-b.", label="data")
    plt.plot([np.max(I)] * 5 + [(np.mean(I))] * 12 + [np.min(I)] * 4, "k-", label="Goal")
    plt.plot(Q, "-r.", label="data")
    plt.plot([np.max(Q)] * 5 + [(np.mean(Q))] * 12 + [np.min(Q)] * 4, "k-", label="Goal")
    plt.ylabel("I+Q [a.u.]")
    plt.xticks(ticks=range(len(sequence)), labels=[str(el) for el in sequence], rotation=45)
    plt.legend()
    plt.show()
