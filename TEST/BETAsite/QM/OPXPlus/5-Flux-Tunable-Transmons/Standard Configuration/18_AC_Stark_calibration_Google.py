...
anharmonicity = eval(f"anharmonicity_q{qubit}")
multiplexed = [1,2,3,4,5]

n_avg = 300
# Detuning to compensate for the AC STark-shift
detunings = np.arange(-10e6, 10e6, 0.2e6)
...
with program() as ac_stark_shift:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(it, iters)):  # QUA for_ loop for sweeping the number of pulses
            ...
            state, I, Q = readout_macro(f"rr{qubit}", multiplexed, threshold=ge_threshold, state=state, I=I, Q=Q)
    ...
    for det in detunings:
        ...
        x180_wf, x180_der_wf = np.array(
            drag_gaussian_pulse_waveforms(
                eval(f"pi_amp_q{qubit}"), pi_len, pi_sigma, alpha=drag_coef, anharmonicity=anharmonicity, detuning=det
            )
        )
        x180_I_wf = x180_wf
