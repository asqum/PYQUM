...
from .tracker.operation_tracker import OperationTracker
from .tracker.sequence_tracker import SequenceTracker
...
class TwoQubitRb:
    ...
    def __init__(
        ...
        self._rb_baker = RBBaker(config, single_qubit_gate_generator, two_qubit_gate_generators, interleaving_gate, operation_tracker)
    ...
    def _insert_all_input_stream(
        ...
        for sequence_depth in sequence_depths:
            for repeat in range(num_repeats):
                # sequence = self._gen_rb_sequence(sequence_depth)  # commands IDs (up to 750)
                # sequence = [99, 733, 123, 727]
                sequence = [19,721,27,732,2,733]   # TQ gate without CZ
