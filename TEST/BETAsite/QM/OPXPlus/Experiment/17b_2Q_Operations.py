...
with program() as cz:
    ...
    with for_(n, 0, n < n_avgs, n+1):
        ...
        with for_(*from_array(phi, Phi)):
            ...
            play("x90_ft", "q2_xy")
            align()
            measure("readout"*amp(1.0), "rr1", None, dual_demod.full("rotated_cos", "out1", "rotated_minus_sin", "out2", I_g[0]),
            dual_demod.full("rotated_sin", "out1", "rotated_cos", "out2", Q_g[0]))
            save(I_g[0], I_st_g[0])
            save(Q_g[0], Q_st_g[0])
            measure("readout"*amp(1.0), "rr2", None, dual_demod.full("rotated_cos", "out1", "rotated_sin", "out2", I_g[1]),
            ...
            play("x90_ft", "q2_xy")
            align()
            measure("readout"*amp(1.0), "rr1", None, dual_demod.full("rotated_cos", "out1", "rotated_minus_sin", "out2", I_e[0]),
            dual_demod.full("rotated_sin", "out1", "rotated_cos", "out2", Q_e[0]))
            save(I_e[0], I_st_e[0])
            save(Q_e[0], Q_st_e[0])
            measure("readout"*amp(1.0), "rr2", None, dual_demod.full("rotated_cos", "out1", "rotated_sin", "out2", I_e[1]),
