...
# Plotting
def live_2D_plotting(n_avg, q_id, job, results, x_range, y_range, title, stage="", normalize=True):
    # Live plotting
    fig = plt.figure()
    ...
    while results.is_processing():
        ...
        for i in q_id:
            ...
            plt.cla()
            plt.title("q%s:"%(i+1))
            if q_id.index(i)==0: plt.ylabel(y_label)
            plt.pcolor(x_var, y_var, map_top)
            plt.axhline(resonator_IF[i] / u.MHz, color="k")
