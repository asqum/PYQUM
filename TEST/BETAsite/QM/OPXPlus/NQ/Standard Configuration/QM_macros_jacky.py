...
def multiRO_pre_save( iqdata_stream, resonators:list, buffer_len ):
    """
    ...
    for idx, res in enumerate(resonators):
        I_st[idx].buffer(buffer_len).average().save(f"{res}_I")
        Q_st[idx].buffer(buffer_len).average().save(f"{res}_Q")
