...
def squarewave(seg0, seg1):
    ...
    i = 1
    if i:
        stat = AWG.CreateArbWaveform(s, ([i*1 for i in range(seg0)] + [i*0 for i in range(seg1)] + [i*1 for i in range(seg0)]))
        Seq = {}
        Seq[(stat[1])] = 1 #loopcount
    ...
    i = 1
    if i:
        stat = AWG.CreateArbWaveform(s, ([i*0 for i in range(seg1)] + [i*0 for i in range(seg0)] + [i*0 for i in range(seg1)]))
        Seq2 = {}
        Seq2[(stat[1])] = 1 #loopcount
