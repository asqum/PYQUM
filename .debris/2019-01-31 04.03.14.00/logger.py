...
def translate_scpi(Name, instance, a, b):
    ...
    if action[0] == 'Set':
        ...
        for i in range(len(parakeys)):
            ...
            elif ' ' in str(action[i+1]): #set parameters for each header by certain parakey
                actionwords = str(action[i+1]).split(' ')
                for j,h in enumerate(headers): 
                    if actionwords[0].upper() in h.upper(): #only need to type part of the header(core)!
                        headers[j] = h.upper() + actionwords[1]
                        paravalues.append(headers[j])
