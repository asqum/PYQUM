...
def search_value(usrdata, val, prepath=()):
...
def search_allvalues(usrdata, val):
    '''return the paths that contain val
    '''
    result, Paths = "", []
    while result is not None:
        result = search_value(usrdata, val)
...
def test_logger():
    ...
    for x in [100, 200, 300, 'David', 'is good']:
        # DATA = Test_DATA
        paths = search_allvalues(Test_DATA, x)
        print("%s result(s) FOUND for %s:\n" %(len(paths), x))
        for p in paths:
