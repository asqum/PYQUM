...
def sweep(bench, start, stop, datapts, sweeptime=0.5, pulsewidth=0.2, vrang='R5'):
    '''
    sweeprate in V/s
    pulsewidth: waiting/staying/settling/stabilization time in sec
    Voltage Range:
        R2: 10mV; R3: 100mV; R4: 1V; R5: 10V; R6: 30V
    '''
    bench.write(vrang) #Voltage range
    GPIBspeed = 62 #pts/s
    swepoints = round(GPIBspeed * sweeptime / datapts)
    Vdata = linspace(start, stop, datapts)
    # set initial sweep here <>
    for i, V in enumerate(Vdata):
        v_prev = float(previous(bench))
        try:
            for v in linspace(v_prev, V, swepoints):
                print("v: %.5f" %v)
                bench.write('S%.5fE'%v) #Set Voltage
            if eval(debugger): print(Fore.YELLOW + "Settling %.5fV..." %V)
            sleep(pulsewidth)
        except:
            pass
...
def test(detail=False):
    ...
    if eval(debugger):
        ...
        output(s, 1)
        sweep(s, 0, 5, 10, 100, 1)
        sweep(s, 5, 0, 10, 100, 1)
        output(s, 0) 
