...
def sdata(bench, format='Real32'):
    if format.lower() == 'real32':
    ...
    IQdata = datas.reshape(sweep(bench)[1]['POINts'], 2)
    Idata, Qdata = IQdata[:,0], IQdata[:,1]
    print(Back.GREEN + Fore.WHITE + "transferred from %s: Data/Idata: %s/%s" %(mdlname, len(datas), len(Idata)))
    return datype
