...
# Setting Trace
Mname = []
Mparam = ['S11', 'S12', 'S21', 'S22']
for S in Mparam:
    iTrace = Mparam.index(S) + 1
    Mname.append("NCHUQ_%s"%S)
    pna.write("CALC:PAR:EXT '%s','%s'" % (Mname[iTrace - 1], S)) #setting measurement name
    pna.write(":DISP:WIND%s:STATe ON" %iTrace)
    pna.write("DISP:WIND%d:TRAC%d:FEED '%s'" % (iTrace, iTrace, Mname[iTrace - 1])) #displaying trace (no set-trace for old-type handling of traces)
    pna.write(":DISP:WIND%d:TRAC%d:Y:AUTO"%(iTrace, iTrace)) #pre-auto-scale
...
for i in range(len(Mparam)):
    pna.write(":DISP:WIND%d:TRAC%d:Y:AUTO"%(i+1, i+1))
...
for i in range(len(Mparam)):
    ...
    pna.write('CALC%s:PAR:SEL %s'%(Mname[i][-2], Mname[i]))
    print("%s on channel # %s is selected!" %(Mname[i], Mname[i][-2]))
