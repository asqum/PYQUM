...
def test(detail=False):
    ...
    if detail:
        ...
        stat = CreateArbWaveform(s, ([i*0 for i in range(1000)] + [i*1 for i in range(3000000)] + [i*0 for i in range(1000)]))
        h1 = stat[1]
        ...
        if i:
            ...
            operation_mode(s, RepCap='2', action=["Set", 1])
            trigger_source_adv(s, RepCap='2')
            trigger_source_adv(s, RepCap='2', action=["Set", 1])
