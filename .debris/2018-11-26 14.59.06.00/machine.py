...
def dso():
    return render_template("blog/machn/dso.html")
...
def dsosettings():
    ...
    stat = DSO.channel1(dsobench, action=['Set', 'DC', '1.56', rnge, scal, ofset, 'OFF'])
    message += ['frequency (GHz): ' + stat[0]]
    powa = request.args.get('powa')
    stat = DSO.power(dsobench, action=['Set',float(powa)])
    message += ['power (dBm): ' + stat[0]]
    oupt = request.args.get('oupt')
    stat = DSO.output(dsobench, action=['Set',int(oupt)])
    message += ['RF output: ' + stat[0]]
    return jsonify(message=message)
...
def dsoabout():
    ...
    message += ['Model: %s (%s)' % (status[1], status[0])]
    status = DSO.frequency(dsobench) # frequency
    message += ['Frequency: %s (%s)' % (status[1], status[0])]
    status = DSO.power(dsobench) # power
    message += ['Power: %s (%s)' % (status[1], status[0])]
    status = DSO.output(dsobench) # output
    message += ['RF output: %s (%s)' % (output_code(status[1]), status[0])]
    return jsonify(message=message)
