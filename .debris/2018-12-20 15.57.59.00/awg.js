...
jQuery.fn.extend({
    marker: function () {
        $.getJSON('/mach/awg/settings-marker', {
            ...
            $('div.awgcontent#debug').append($('<h4 class="dh4" style="background-color: lightgreen;"></h4>').text(Date($.now())));
            $.each(data.message, function(index, value) {$('div.awgcontent#debug').append($('<h4 class="dh4" style="color: black;"></h4>').text(Number(index+1) + ". " + value));});
...
$(function () {
    $('input.awg#submit-all').bind('click', function () { // the enter key code
        // $.getJSON('/mach/awg/settings-marker', {
        ...
        //     $('div.awgcontent#debug').append($('<h4 class="dh4" style="background-color: lightgreen;"></h4>').text(Date($.now())));
        //     $.each(data.message, function(index, value) {
        //         $('div.awgcontent#debug').append($('<h4 class="dh4" style="color: black;"></h4>').text(Number(index+1) + ". " + value));
        //     });
        //     $(".dh4").wrapAll("<div class='container'></div>");
        //     $('button.awg#about').trigger('click'); //click on about //or: .click();
        // });
