...
from numpy import arange, sqrt, arctan2, array, linspace, pi, log10, reshape, unwrap, gradient, ones, convolve, ceil
...
UPHA = unwrap(Pha)
avebox = int(ceil(len(X)/100))
UPHA_smooth = smooth(UPHA, avebox)
UPHA_flat = gradient(UPHA_smooth, X)
UPHA_flat = minmax_scale(UPHA_flat)

f, spec, UPHA_fft = FFT_deNoise(UPHA_flat[avebox:-avebox]*1e8, X[1]-X[0], 0.0001)
