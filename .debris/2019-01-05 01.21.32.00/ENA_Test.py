import visa
from numpy import arange, sqrt, arctan
...
def setrace(chnum, trace_num):
    status = ena.write('CALC%s:PAR:COUN %s' %(chnum, trace_num)) #set 2 traces
    print('we have set %s trace %s' %(trace_num, [x for x in status]))
    trace_info, Mname = '', {}
...
ena.write("SENSe:SWEep:POINts 600")
...
IFB = (float(fstart) - float(fstop))/600/10
...
# start measurement
status = ena.write("INIT1:CONT ON;TRIG:SOUR INT;*OPC?")
# status = ena.write("INIT:CONT OFF;INIT:IMM;") #
...
status = ena.write("FORMat:DATA ASCii")
print("Format data: %s" %[x for x in status])

chnum = 1
# select measurement name
status = ena.write('CALC%d:PAR:SEL'%(chnum))
