...
def display2D(dx, y, units, channel):
    ...
    svg_file = "%swaveform(CH%s).svg" %(mdlname, channel)
    SVG = Path(INSTR_PATH) / svg_file
    # Organizing Data
    Y = [float(i) for i in y.split(",")[1:-1]] # to avoid the first and the last string
    X = range(len(Y)) / len(Y) * dx #X = arange(0, len(Y), 1) * dx
    # Plotting
    fig, ax = plt.subplots()
    ax.plot(X, Y)
    ax.set(xlabel='time(%s)'%units[0], ylabel='voltage (%s)'%units[1], title=mdlname+'-Waveform (CH%s)'%channel)
