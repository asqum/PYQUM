...
from pyqum.instrument.reader import smooth, FFT_deNoise
...
# Sorting paths
tstamp0 = [t[0] for t in p]
print(tstamp0)

tstamp = [datetime.strptime(i, '%a %b %d %H:%M:%S %Y') for i in tstamp0]
ilatest, latest = max(enumerate(tstamp), key=itemgetter(1))
print("The latest being %s in %s-th path: %s" %(latest, ilatest, tstamp0[ilatest]))

selectedP = p[ilatest] #[8]
...
f, spec, UPHA_fft = FFT_deNoise(UPHA_flat[avebox:-avebox]*1e8, X[1]-X[0], 0.00001)

indices = indexes(UPHA_fft, thres=0.7, min_dist=0.1)
...
ax.plot(X_peak, Y_peak, marker='P', color='r', markersize=17)
ax.set(ylabel=r' unwrapped Phase (rad)', xlabel=r'$frequency (Hz)$',
