...
Ave_num = 1
status = ena.write("SENSe:SWE:GRO:COUN %s" %Ave_num) #Averaged by VNA itself
print("Set Trigger group count=%s: %s" %(Ave_num, [x for x in status]))
status = ena.write("TRIG:SOUR IMM")
...
status = ena.query("CALC1:PAR:CAT?")
...
status = ena.write('CALC%d:DATA SDATA %s'%(chnum, Mname))
print("select sdata: %s" %[x for x in status])

data = ena.query("CALC%d:DATA? SDATA"%chnum)
# print(data)
datas = data.split(',')
