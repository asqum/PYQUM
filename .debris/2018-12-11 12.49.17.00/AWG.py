...
def CreateArbSequence(session, sequence):
    ...
    Size = len(sequence)
    print("size: %s" %Size)
    wfmhandles = (c_long * Size)(*[int(i) for i in sequence.keys()])
    print("Sequence.keys: %s" %[int(i) for i in sequence.keys()])
    loopcounts = (c_long * Size)(*[int(i) for i in sequence.values()])
    print("Sequence.values: %s" %[int(i) for i in sequence.values()])
...
def test(detail=False):
    ...
    if detail:
        ...
        stat = CreateArbWaveform(s, ([i*0 for i in range(1)] + [i*1 for i in range(3500)] + [i*0 for i in range(1)]))
        h1 = stat[1]
        stat = CreateArbWaveform(s, list([i*0 for i in range(3500)]))
        h2 = stat[1]
