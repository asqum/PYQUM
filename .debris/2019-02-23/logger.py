...
class measurement:
    ...
    def log(self, data, resume=False):
        ...
        usr_bag = bytes("{'%s': {'%s': {'%s': {'%s': {'c-order': %s, 'data': %s}}}}}" %(ctime(), self.place, mission, task, self.corder, self.data), 'utf-8') #serialize the json
        ...
        if self.existence == False:
            ...
            with open(self.pqfile, 'rb+') as ufile:
                ...
                ufile.write(bytes(", ", 'ascii') + usr_bag[1:-1] + bytes("}", 'ascii'))
        return
