...
def address(instr_name, reset=False):
    ...
    rs["VSA"] = "PXI27::0::0::INSTR;PXI22::12::0::INSTR;PXI22::14::0::INSTR;PXI22::8::0::INSTR;PXI22::9::0::INSTR"
...
class measurement:
    ...
    '''
    def __init__(self, directive, corder, data=[], usr_name='USR', place='Unknown', sample='Sample'):
    ...
    def log(self, data, resume=False):
        ...
        usr_bag = str({ctime(): {self.place: {mission: {task: 0}}}}) #serialize the json

        if self.existence == False:
            # Initialize blank file
            with open(self.pqfile, 'rb+') as jfile:
                jfile.write(bytes(usr_bag, 'utf-8'))
        else:
            with open(self.pqfile, 'rb+') as ufile: # truncate the last letter from file
                ufile.seek(-1, SEEK_END)
                ufile.truncate()
            # with open(self.pqfile, 'rb+') as jfile: #paste new data
                ufile.write(bytes(", ", 'ascii') + bytes(usr_bag[1:-1], 'ascii') + bytes("}", 'ascii'))
        return
