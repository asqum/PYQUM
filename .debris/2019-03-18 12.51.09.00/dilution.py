...
class bluefors:
    ...
    def checkvalves(self):
        tell = Telnet("192.168.1.23",8325)
        # tell.interact()
        tell.write("remote\n".encode('ascii'))
        output = tell.read_until(b"\n")
        print(output)
        tell.write("control 1\n".encode('ascii'))
        output = tell.read_until(b"\n")
        print(output)
        tell.write("remote 1\n".encode('ascii'))
        output = tell.read_until(b"\n")
        print(output)
        tell.write("status v17\n".encode('ascii'))
        output = tell.read_until(b"\n")
        print(output)
        tell.write("status scroll2\n".encode('ascii'))
        output = tell.read_until(b"\n")
        print(output)
        tell.write("mgstatus 2\n".encode('ascii'))
        output = tell.read_until(b"\n")
        print(output)
        tell.write("on scroll2\n".encode('ascii'))
        output = tell.read_until(b"\n")
        print(output)
        if int(input("OFF SCROLL2? (1/0")):
            tell.write("off scroll2\n".encode('ascii'))
            output = tell.read_until(b"\n")
            print(output)
        tell.write("exit\n".encode('ascii'))
        output = tell.read_until(b"\n")
