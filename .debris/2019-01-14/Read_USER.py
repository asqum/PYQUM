...
def FFT_deNoise(y, dx, noise_level):
    ...
    f = rfftfreq(len(y), dx)
    spectrum = w**2
    cutoff = spectrum < (spectrum.max()*noise_level)
    w2 = w.copy()
    w2[cutoff] = 0
    y_clean = irfft(w2)
...
UPHA = unwrap(Pha)
UPHA_flat = gradient(UPHA, X)

avebox = 700
UPHA_smooth = smooth(UPHA_flat, avebox)

f, spec, UPHA_fft = FFT_deNoise(UPHA_smooth, X[1]-X[0], 0.1)
